...
export class Preserve extends BaseHandler {
	...
	public fillEmbed(embed: MessageEmbed): void {
		embed
            ...
            \`1\` means that messages should not be deleted once the Voice Channel is empty, \`0\` - delete all unpinned messages after the last User has left the Voice Channel (0 is the default option for each Text Channel the bot creates).
		...
		`)
			.addField('Examples', `
			\`${this.cmd} 1 717824008636334130\` - request to keep message in the Text Channel that is mapped to a Voice Channel with the ID \`717824008636334130\`
			\`${this.cmd} 0 717824008636334130\` - request to resume message deletion in the Text Channel that is mapped to a Voice Channel with the ID \`717824008636334130\`
		`)
	}
}